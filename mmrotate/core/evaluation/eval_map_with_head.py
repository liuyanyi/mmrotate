...
def tpfp_default(det_bboxes,
    ...
    det_bboxes, det_head_cls = np.array(det_bboxes[0]), np.array(det_bboxes[1])
    gt_bboxes, gt_heads = gt_bboxes[0], gt_bboxes[1]
...
def get_cls_results(det_results, annotations, class_id):
    ...
    for ann in annotations:
        ...
        head_class = head2class(ann['headers'][gt_inds, :], ann['bboxes'][gt_inds, :])
        cls_gts.append((ann['bboxes'][gt_inds, :], head_class))
...
def eval_rbbox_head_map(det_results,
    ...
    for i in range(num_classes):
        ...
        for _, (bbox, _) in enumerate(cls_gts):
            if area_ranges is None:
...
def print_map_summary(mean_ap,
    ...
    header = ['class', 'gts', 'dets', 'recall', 'ap', 'head_acc']
    for i in range(num_scales):
        if scale_ranges is not None:
