...
class GFLModeSwitchHook(Hook):

    def __init__(self, start_epochs=2):
    ...
    def filter_bboxes(self, cls_scores, bbox_preds):
        ...
        for lvl in range(num_levels):
            ...
            for img_id in range(num_imgs):
                best_point_ind_i = best_point_ind[img_id]
                best_pred_i = best_pred[img_id]
                best_point_i = points.gather(
                    dim=-2, index=best_point_ind_i).squeeze(dim=-2)
                best_bbox_i = self.bbox_coder.decode(best_point_i, best_pred_i)
